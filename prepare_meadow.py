...
import datasets # for downloading medical-meadow

def prepare(
    destination_path: Path = Path("/home/t_goto/lit-gpt/data/medical-meadow"),
    checkpoint_dir: Path = Path("checkpoints/meta-llama/Llama-2-7b-chat-hf"),
    test_split_fraction: float = 0.03865,  # to get exactly 2000 test samples,
    seed: int = 42,
    mask_inputs: bool = False,  # as in alpaca-lora
    data_file_name: str = "medical_meadow_medqa.json",
    ...
    dataset = datasets.load_dataset('medalpaca/medical_meadow_medqa', split="train")
    dataset.to_json("data/medical-meadow/medical_meadow_medqa.json")
    data_files = {"train": "data/medical-meadow/medical_meadow_medqa.json"}
    med_data = datasets.load_dataset("json", data_files=data_files, split="train")

    # Partition the dataset into train and test
    train_set, test_set = random_split(
        med_data, [1.0 - test_split_fraction, test_split_fraction], generator=torch.Generator().manual_seed(seed)
    )
